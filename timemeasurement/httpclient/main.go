...
func main() {
	...
	images := 20
	var benchmark_convert_jpg = make([]int, images)
	var benchmark_convert_webp = make([]int, images)
	var benchmark_retrieve_jpg = make([]int, images)
	var benchmark_retrieve_webp = make([]int, images)
	...
	for k := 1; k <= 4; k++ {
		...
		for i := 1; i <= images; i++ {
			...
			benchmark_convert_jpg[i-1] = int(elapsed)
			...
			elapsed = int(time.Since(start))
			benchmark_retrieve_jpg[i-1] = int(elapsed)
			...
			elapsed = int(time.Since(start))
			benchmark_convert_webp[i-1] = int(elapsed)
			//Stream a file from the server
			start = time.Now()
			...
			elapsed = int(time.Since(start))
			benchmark_retrieve_webp[i-1] = int(elapsed)
		}
		...
		for i := 1; i <= images; i++ {
			_, _ = datawriter.WriteString(strconv.Itoa(i) + "\t" + strconv.Itoa(benchmark_convert_jpg[i-1]) + "\t" + strconv.Itoa(benchmark_retrieve_jpg[i-1]) + "\n")
		}
		datawriter.Flush()
		file.Close()
		...
		for i := 1; i <= images; i++ {
			_, _ = datawriter_2.WriteString(strconv.Itoa(i) + "\t" + strconv.Itoa(benchmark_convert_webp[i-1]) + "\t" + strconv.Itoa(benchmark_retrieve_webp[i-1]) + "\n")
		}
		datawriter_2.Flush()
		file_2.Close()
